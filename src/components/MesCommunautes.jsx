...
const MesCommunautes = () => {
  ...
  const joinUs = () => {
    const choice = confirm(
      "En appuyant sur le boutton rejoindre, vous acceptez de vous soumettre aux réglements de la communauté. Appuyer sur OK pour confirmez votre choix afin rejoindre la communauté.",
    );
    if (choice) {
  ...
  const waitAcces = () => {
    const submission = confirm(
      "En appuyant sur le boutton rejoindre vous acceptez de vous soumettre aux réglements de la communauté. Une demande d'addhésion sera envoyée à l'administrateur de la communauté , cliquez sur OK pour envoyer votre demande.",
    );
    if (submission)
      alert(
        "Demande envoyé avec succès, vous receverez une notification concernant votre demande.",
      );
            ...
            <div className="row g-5 mt-2">
              ...
              >
                <div className="d-flex gap-3">
                  ...
                  {!actifCommunity.isPrivate && (
                    <button
                      className="btn btn-primary-custom rounded-5"
                      onClick={joinUs}
                    >
                      Rejoindre
                    </button>
  ...
  return (
    <div>
      <h3 className="my-4">Mes communautés</h3>
      <div className="d-flex gap-3">
        {myCommunities.map((com) => (
          ...
          >
            <div
              className="img-author rounded-pill m-auto"
              style={{ width: "100px", height: "100px" , border: `3px solid ${com.border}`}}
            >
              <img
                src={com.coverImage}
                alt="communauty_avatar"
              />
            </div>
          ...
          <h4 className="my-4">Découvrez ce que vous avez manqué depuis votre dernière visite</h4>
<div>
  {myCommunities
    .flatMap((com) => com.posts) // Récupère tous les posts de chaque communauté et les met dans un seul grand tableau
    .map((p) => (
      <div key={p.id} className="card p-3 mb-3 rounded-4">
        <div className="d-flex gap-4">
          <div className="d-flex gap-2">
            <div className="img-author rounded-5">
              <img src={p.avatar} className="border rounded-5" alt={p.author} />
            </div>
            <p className="my-3">{p.author}</p>
          </div>
          <p className="d-flex my-3">
            <span className="material-symbols-outlined mx-1">favorite_border</span>
            {p.likes}
          </p>
          <p className="d-flex my-3">
            <span className="material-symbols-outlined mx-1">chat_bubble</span>
            25
          </p>
          <p className="d-flex my-3">
            <span className="material-symbols-outlined mx-1">calendar_month</span>
            {p.date}
          </p>
        </div>
        <p>Sujet: {p.title}</p>
